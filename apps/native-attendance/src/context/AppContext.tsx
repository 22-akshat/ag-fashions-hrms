...
type AppContextValue = {
  ...
  faceSessionExpiresAt: string | null;
  punchIntent: PunchIntent | null;
  /** Set after a successful mark for SuccessScreen copy (not persisted). */
  lastSuccessfulPunchType: PunchIntent | null;
  ...
  setFaceSession: (token: string | null, expiresAtIso: string | null) => void;
  setPunchIntent: (intent: PunchIntent | null) => void;
  setLastSuccessfulPunchType: (intent: PunchIntent | null) => void;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [faceSessionExpiresAt, setFaceSessionExpiresAt] = useState<string | null>(null);
  const [punchIntent, setPunchIntent] = useState<PunchIntent | null>(null);
  const [lastSuccessfulPunchType, setLastSuccessfulPunchType] = useState<PunchIntent | null>(null);
  ...
  const setFaceSession = useCallback((token: string | null, expiresAtIso: string | null) => {
    setFaceSessionToken(token);
    setFaceSessionExpiresAt(expiresAtIso);
  }, []);
  ...
  const value = useMemo<AppContextValue>(
    () => ({
      ...
      faceSessionToken,
      faceSessionExpiresAt,
      punchIntent,
      lastSuccessfulPunchType,
      setStage,
      setEmployeeId,
