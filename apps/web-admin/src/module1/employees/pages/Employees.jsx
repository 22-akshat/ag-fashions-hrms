...
export function Employees() {
  ...
  async function provisionEmployeeAuth() {
    if (!supabase || !detailTarget?.id) return
    const phone = String(authPhoneDraft ?? '').trim()
    const password = String(authPasswordDraft ?? '')
    if (!phone || password.length < 6) {
      toast({ title: 'Invalid input', description: 'Phone is required and password must be at least 6 characters.', variant: 'destructive' })
      return
    }
    setAuthProvisionBusy(true)
    try {
      const { data, error } = await supabase.functions.invoke('hr-provision-employee-auth', {
        body: { employee_id: detailTarget.id, phone, password, status: 'active' },
      })
      if (error) throw error
      ...
      toast({ title: 'Provision failed', description: String(e?.message || e), variant: 'destructive' })
    } finally {
        ...
        <DialogContent className="max-h-[min(90vh,760px)] max-w-4xl overflow-y-auto">
          ...
          {detailTarget ? (
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-border/60 bg-white p-4">
                <p className="text-sm font-semibold text-foreground">Employee mobile login</p>
                <p className="text-xs text-muted-foreground">
                  Provisions a Supabase Auth user (phone + password) and links it to this employee.
                </p>
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <Label>Phone (from phone_no_1)</Label>
                    <Input value={authPhoneDraft || authPhoneDefault} onChange={(e) => { setAuthProvisionResult(null); setAuthPhoneDraft(e.target.value) }} placeholder="+91…" />
                  </div>
                  <div>
                    <Label>Temporary password</Label>
                    <Input value={authPasswordDraft} onChange={(e) => { setAuthProvisionResult(null); setAuthPasswordDraft(e.target.value) }} placeholder="Min 6 chars" />
                  </div>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button type="button" disabled={authProvisionBusy} onClick={provisionEmployeeAuth}>
                    {authProvisionBusy ? 'Provisioning…' : 'Provision login'}
                  </Button>
                </div>
                {authProvisionResult?.auth_user_id ? (
                  <p className="mt-2 text-xs text-muted-foreground">Auth user id: {authProvisionResult.auth_user_id}</p>
                ) : null}
              </div>
            </div>
          ) : null}
